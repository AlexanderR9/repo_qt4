...
void LChartWidget::convertPoint(const QPointF &p1, QPointF &p2)
{
    ...
    //double x2 = m_axis.zero_point.x() + m_axis.xLen*dpx;
    //double y2 = m_axis.zero_point.y() - m_axis.yLen*dpy;
    //p2.setX(x2);
    //p2.setY(y2);
}
...
//	list.append(min.x() + (i+1)*d*factor);
...
//    qDebug()<<QString("marksYInterval=%1  yLen=%2  n_marks=%3").arg(marksYInterval).arg(yLen).arg(n_marks);
    for (int i=0; i<n_marks; i++)
//	list.append(min.y() + (i+1)*d*factor);
